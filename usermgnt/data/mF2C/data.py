...
def get_current_device_id():
    LOG.info("[usermgnt.data.mF2C.data] [get_current_device_id] Getting 'my' device ID from 'agent' resource ...")
    ...
    # get from AGENT resource
        ...
        if not agent is None and agent != -1:
            LOG.info("[usermgnt.data.mF2C.data] [get_current_device_id] Getting device 'id' by 'deviceID'=" + agent['device_id'])
...
def get_agent_info():
    LOG.info("[usermgnt.data.mF2C.data] [get_agent_info] Getting 'agent' resource ...")
    # get from AGENT resource
    agent = cimi.get_agent_info()
...
def update_sharing_model_by_id(sharing_model_id, data):
    sharing_model_id = sharing_model_id.replace('sharing-model/', '')
    LOG.info("[usermgnt.data.mF2C.data] [update_sharing_model_by_id] " + sharing_model_id + ", " + str(data))
...
def delete_sharing_model_by_id(sharing_model_id):
    sharing_model_id = sharing_model_id.replace('sharing-model/', '')
    LOG.info("[usermgnt.data.mF2C.data] [delete_sharing_model_by_id] " + sharing_model_id)
    resp = cimi.get_resource_by_id("sharing-model/" + sharing_model_id)
    if resp and resp == -1:
        return -1
    elif resp:
...
def get_user_profile_by_id(profile_id):
    ...
    LOG.debug("[usermgnt.data.mF2C.data] [get_user_profile_by_id] " + profile_id)
    return cimi.get_resource_by_id("user-profile/" + profile_id)


# get_user_profile: Get user profile
def get_user_profile(device_id):
    LOG.debug("[usermgnt.data.mF2C.data] [get_user_profile] device_id=" + device_id)
...
def update_user_profile_by_id(profile_id, data):
    profile_id = profile_id.replace('user-profile/', '')
    LOG.debug("[usermgnt.data.mF2C.data] [update_user_profile_by_id] " + profile_id + ", " + str(data))
...
def delete_user_profile_by_id(profile_id):
    profile_id = profile_id.replace('user-profile/', '')
    LOG.debug("[usermgnt.data.mF2C.data] [delete_user_profile_by_id] Delete Profile [" + profile_id + "]")
    resp = cimi.get_resource_by_id("user-profile/" + profile_id)
    if resp and resp == -1:
        return None
    elif resp:
...
def register_user(data):
    LOG.debug("[usermgnt.data.mF2C.data] [register_user] Creating new Profile for user [data=" + str(data) + "] ...")
...
def get_power():
    ...
    LOG.info("[usermgnt.data.mF2C.data] [get_power] Getting power status from device [" + device_id + "] ...")
