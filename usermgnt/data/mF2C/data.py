...
def get_current_device_id():
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_current_device_id] Getting 'my' device ID from 'agent' resource ...")
    # get from local volume
    device_id = vol.read_device_id()
    if device_id is not None and not device_id == "" and len(device_id) > 0:
        LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_current_device_id] (LOCAL VOLUME) device_id = " + device_id)
        return device_id
    # get from AGENT resource
    else:
        agent = cimi.get_agent_info()
        LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_current_device_id] agent=" + str(agent))
        if not agent is None and agent != -1:
            LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_current_device_id] Getting device 'id' by 'deviceID'=" + agent['device_id'])
            id = cimi.get_id_from_device(agent['device_id'])
            if not id is None and id != -1:
                LOG.info("[usermgnt.data.mF2C.data] [get_current_device_id] Returning 'my' device ID = " + id)
                return id
            else:
...
def get_agent_info():
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_agent_info] Getting 'agent' resource ...")
    # get from AGENT resource
    agent = cimi.get_agent_info()
    LOG.debug("[usermgnt.data.mF2C.data] [get_agent_info] agent = " + str(agent))
    if not agent is None and agent != -1:
        return agent
    else:
        return -1
...
def get_user_info(user_id):
    ...
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_user_info] Getting info from user " + user_id + " ...")
...
def delete_user(user_id):
    user_id = user_id.replace('user/', '')
    LOG.info("[usermgnt.data.mF2C.data] [delete_user] Removing user [" + user_id + "] from mF2C ...")
...
def get_sharing_model_by_id(sharing_model_id):
    ...
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_sharing_model_by_id] " + sharing_model_id)
    return cimi.get_resource_by_id("sharing-model/" + sharing_model_id)


# Get shared resources
def get_sharing_model(device_id):
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_sharing_model] " + device_id)
    return cimi.get_sharing_model(device_id)


# Initializes shared resources values
def init_sharing_model(data):
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [init_sharing_model] " + str(data))
...
def update_sharing_model_by_id(sharing_model_id, data):
    sharing_model_id = sharing_model_id.replace('sharing-model/', '')
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [update_sharing_model_by_id] " + sharing_model_id + ", " + str(data))
...
def delete_sharing_model_by_id(sharing_model_id):
    sharing_model_id = sharing_model_id.replace('sharing-model/', '')
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [delete_sharing_model_by_id] " + sharing_model_id)
    resp = cimi.get_resource_by_id("sharing-model/" + sharing_model_id)
    if resp and resp == -1:
        return -1
    elif resp:
...
def get_current_sharing_model():
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_current_sharing_model] Getting information about current user and device ...")

    device_id = get_current_device_id()  # get 'my' device_id from 'agent' resource
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_current_sharing_model] device_id=" + str(device_id))
...
def get_user_profile_by_id(profile_id):
    ...
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_user_profile_by_id] " + profile_id)
    return cimi.get_resource_by_id("user-profile/" + profile_id)


# get_user_profile: Get user profile
def get_user_profile(device_id):
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_user_profile] device_id=" + device_id)
...
def update_user_profile_by_id(profile_id, data):
    profile_id = profile_id.replace('user-profile/', '')
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [update_user_profile_by_id] " + profile_id + ", " + str(data))
...
def delete_user_profile_by_id(profile_id):
    profile_id = profile_id.replace('user-profile/', '')
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [delete_user_profile_by_id] Delete Profile [" + profile_id + "]")
    resp = cimi.get_resource_by_id("user-profile/" + profile_id)
    if resp and resp == -1:
        return None
    elif resp:
...
def register_user(data):
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [register_user] Creating new Profile for user [data=" + str(data) + "] ...")
...
def get_current_user_profile():
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_current_user_profile] Getting information about current user and device ...")

    device_id = get_current_device_id() # get 'my' device_id from 'agent' resource
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_current_user_profile] device_id=" + str(device_id))
...
def get_total_services_running():
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_total_services_running] Total of services running in device = " + str(config.APPS_RUNNING))
...
def get_power():
    ...
    LOG.log(TRACE, "[usermgnt.data.mF2C.data] [get_power] Getting power status from device [" + device_id + "] ...")
